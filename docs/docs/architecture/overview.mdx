...
description: How the open-source CLI talks to Ollama locally and to QuickSlug Cloud when needed.
---

# Architecture overview

QuickSlug is an **open-core monorepo**. The MIT-licensed pieces live in [`quickslug-public-cli/`](https://github.com/quickslug/quickslug/tree/main/quickslug-public-cli) and ship to every customer. The proprietary cloud control plane (`quickslug-core-backend/`) is hosted by us and never bundled with the CLI.

## Component map

```text
                       ┌────────────────────────┐
                       │   quickslug-public-cli │  (this repo, MIT)
                       │                        │
        ┌──────────────┤  src/cli/   (Commander)│
        │              │  src/api/   (Fastify)  │
        │              │  src/worker/(in-proc)  │
        ...
        │                        ▼
        │              ┌────────────────────────┐
        │              │   QuickSlug Cloud      │  (proprietary, hosted)
        │              │   /api/inference       │
        │              │   /api/train           │
        │              │   /api/auth/*          │
        │              │   /api/usage           │
...
| `src/api/` | Fastify 5 HTTP gateway. Routes: `/v1/chat/completions`, `/v1/completions`, `/v1/models`, `/v1/quickslug/health`, `/v1/quickslug/training/*` |
...
`local` streams from `<OLLAMA_HOST>/api/chat`. `remote` proxies to `POST /api/inference` on QuickSlug Cloud (which handles RunPod). `not_found` returns `404 model_not_found`.
...
The local API's `/v1/quickslug/training/*` handlers inspect the `Authorization` header and act in one of two modes:

| Header | Behaviour |
|---|---|
| `Bearer qs_…` | Proxies the request verbatim to QuickSlug Cloud's `/api/train*`. Surfaces upstream `403 pro_subscription_required` as-is. |
...
The `quickslug train --cloud` flag bypasses the local API entirely and calls the cloud directly — that's why the local-API auto-proxy path exists separately, for direct API consumers (HTTP clients other than the CLI).

## Local data model (SQLite)

Created by [`quickslug init`](../cli/init) at `~/.quickslug/db.sqlite`:
...
`training_jobs` is written by the worker. **`inference_logs` is currently unused in the public CLI v1.0** — the chat handler computes per-request usage but the local `tracker.record()` is a no-op stub (see [`src/api/services/localStubs.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/api/services/localStubs.ts)). Pro-tier inference is metered server-side.
...
| Pro token expired | Cloud returns `401`; CLI surfaces `Token rejected by cloud — run quickslug login` |

## Security boundaries

- The local API binds to `0.0.0.0` by default. Use a reverse proxy if you need TLS or auth in front of it.
- The Pro token in `~/.quickslug/config.json` is `mode 0600` — only the current user can read it.
