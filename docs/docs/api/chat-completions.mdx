...
description: OpenAI-compatible chat-completions endpoint with streaming. Routes to Ollama locally and to QuickSlug Cloud when needed.
---
...
Source: [`src/api/routes/chat.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/api/routes/chat.ts) · [`src/api/services/localRouter.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/api/services/localRouter.ts)
...
| `Authorization: Bearer qs_…` | Pro tier — token forwarded to QuickSlug Cloud on fallback |
...
Source: [`localRouter.resolve()`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/api/services/localRouter.ts):
...
When the resolved provider is `LOCAL`, the API streams from Ollama. When it's `REMOTE`, the request is proxied to `POST /api/inference` on QuickSlug Cloud (which routes to RunPod). When it's `NOT_FOUND`, the handler returns:
...
[`src/api/middleware/rateLimiter.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/api/middleware/rateLimiter.ts) is a per-key in-memory token bucket:

- **Free tier (no API key)** — middleware short-circuits with `if (!apiKey) return;` — no local rate limit.
- **Pro tier** — the local stub assigns `9999 rpm`. The real limit is enforced server-side by QuickSlug Cloud.
...
const client = new OpenAI({
  ...
  apiKey: process.env.QUICKSLUG_KEY ?? 'not-needed',
});
...
The local API computes per-request usage metrics (`tokensIn`, `tokensOut`, `latencyMs`, `ttfbMs`, `status`) but the v1.0 public CLI uses a **no-op tracker** locally — see [`src/api/services/localStubs.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/api/services/localStubs.ts). The `inference_logs` SQLite table is created by `quickslug init` for forward compatibility but is not written to. **Pro-tier usage** is metered server-side by QuickSlug Cloud and reported by [`quickslug usage`](../cli/usage).
