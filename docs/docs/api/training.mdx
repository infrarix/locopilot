...
sidebar_position: 4
title: Training API
description: HTTP routes under /v1/locopilot/training for submitting jobs, polling status, and streaming logs.
---
...
Source: [`src/api/routes/training.ts`](https://github.com/locopilot/locopilot/blob/main/locopilot-public-cli/src/api/routes/training.ts) · [`src/worker/handlers.ts`](https://github.com/locopilot/locopilot/blob/main/locopilot-public-cli/src/worker/handlers.ts)
...
| Anything else (or no header) | The job runs in-process via [`BasicWorker`](https://github.com/locopilot/locopilot/blob/main/locopilot-public-cli/src/worker/) on the local machine. |
...
This is the endpoint that powers [`locopilot logs`](../cli/logs).
