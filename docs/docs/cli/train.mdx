---
sidebar_position: 5
title: locopilot train
description: Submit a fine-tuning job — local in-process (Free) or routed to LocoPilot Cloud (Pro).
---
...
Source: [`src/cli/commands/train.ts`](https://github.com/locopilot/locopilot/blob/main/locopilot-public-cli/src/cli/commands/train.ts)
...
| `--cloud` | ❌ | `false` | Route the job to LocoPilot Cloud instead of running locally. Requires `locopilot login`. |
...
If your terminal disconnects, the job keeps running. Reattach with [`locopilot logs`](logs):
