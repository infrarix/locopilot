...
Source: [`src/cli/commands/doctor.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/cli/commands/doctor.ts)
...
| 3 | **Home directory writable** | Can write a temp file inside `~/.quickslug/` |
| 4 | **SQLite database exists** | `~/.quickslug/db.sqlite` exists and is non-empty |
...
| 8 | **Local API server reachable** | `GET /v1/quickslug/health` responds 200 — i.e. `quickslug start` is running |
...
```text
  ...
  Pro: not logged in — run `quickslug login` to enable cloud features
