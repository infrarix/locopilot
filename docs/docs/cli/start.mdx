---
sidebar_position: 2
title: quickslug start
description: Boot the local Fastify API server.
---

# `quickslug start`

> Start the QuickSlug local API server.

Source: [`src/cli/commands/start.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/cli/commands/start.ts) · [`src/api/index.ts`](https://github.com/quickslug/quickslug/blob/main/quickslug-public-cli/src/api/index.ts)

## Synopsis

```bash
quickslug start [--port <port>]
```
...
1. Loads `~/.quickslug/.env` and aborts with a clear error if `~/.quickslug/db.sqlite` is missing — that means [`quickslug init`](init) has not been run.
...
🐌 QuickSlug
  ...
  Starting QuickSlug on port 8080...

  Entry: /Users/you/.../node_modules/quickslug/dist/api/index.js
...
| `GET  /v1/quickslug/health` | public | Heartbeat (Ollama + SQLite probes; adds `cloud` probe on Pro) |
| `* /v1/quickslug/training/*` | optional | Runs in-process locally; auto-proxies to cloud when `Authorization: Bearer qs_…` is set |

The Fastify pipeline is:

1. `onRequest` → validates `Authorization` (skipped for `/v1/models` and `/v1/quickslug/health`). Non-`qs_` headers are treated as anonymous (Free).
...
**`Database not found.`** — you skipped `quickslug init`. Run it once.
...
**Port already in use** — Fastify will throw `EADDRINUSE`. Pick a different port: `quickslug start --port 8090`.
